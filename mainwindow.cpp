        ...
        if (IS_Data_Recevied)
        {
            ...
            if (query.exec())
            {
                if (query.next())
                {
                    ...
                    {
                        responseMessage = "Bienvenue\n";
                        COMPORT->write(responseMessage.toUtf8());
                        qDebug() << "Response sent To Arduino !! " << "Valid";
                         query.prepare("SELECT mot_de_passe FROM employe WHERE id_emp = :id_emp");
                        query.bindValue(":id_emp", datareceived);
                    ...
                    else
                    {
                        ...
                        qDebug() << "Failed to send response to Arduino: Port not writable.";
                    }
                }
                else
                {
            ...
            else
            {
                ...
                ui->receiveddata->append("Error checking ID.");
            }
